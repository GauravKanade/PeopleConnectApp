...
public class GetPublicQuestions extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String str10 = "SELECT AREA_ID, CITY_ID, STATE_ID FROM USERS WHERE USER_ID=?";
            PreparedStatement prep10 = con.prepareStatement(str10);
            prep10.setString(1, using_id);
            ResultSet rs10 = prep10.executeQuery();
            while (rs10.next()) {
                user_area_id = rs10.getString("AREA_ID");
                user_city_id = rs10.getString("CITY_ID");
                user_state_id = rs10.getString("STATE_ID");
            }

            String str1 = "SELECT * FROM PUBLIC_QUESTIONS WHERE STATE_ID=? OR (STATE_ID=0 AND CITY_ID=?) OR (STATE_ID=0 AND CITY_ID=0 AND AREA_ID=?) OR (STATE_ID=0 AND CITY_ID=0 AND AREA_ID=0) ORDER BY POSTED_ON DESC";
            PreparedStatement prep1 = con.prepareStatement(str1);
            prep1.setString(1, user_state_id);
            prep1.setString(2, user_city_id);
            prep1.setString(3, user_area_id);
